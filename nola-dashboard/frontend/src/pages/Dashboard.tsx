    ...
    <div className="p-6 space-y-4 bg-background">
      ...
      {!loading && !error && metrics && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard title="Faturamento Total" value={currency(metrics.total_revenue)} icon={<DollarSign className="h-6 w-6" />} />
          <MetricCard title="Total de Pedidos" value={metrics.total_orders} icon={<svg className="h-6 w-6" stroke="currentColor" fill="none"><circle cx="8" cy="21" r="1" /><circle cx="19" cy="21" r="1" /><path d="M2.05 2.05h2l2.66 12.42a2 2 0 002 1.58h9.78a2 2 0 001.95-1.57L22 7H5.12" /></svg>} />
          <MetricCard title="Valor Médio do Pedido" value={currency(metrics.avg_ticket)} icon={<svg className="h-6 w-6" stroke="currentColor" fill="none"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17" /><polyline points="16 7 22 7 22 13" /></svg>} />
          <MetricCard title="Lojas Ativas" value={metrics.active_stores} icon={<svg className="h-6 w-6" stroke="currentColor" fill="none"><path d="m2 7 4.41-4.41A2 2 0 017.83 2h8.34a2 2 0 011.42.59L22 7" /><path d="M4 12v8a2 2 0 002 2h12a2 2 0 002-2v-8" /><path d="M15 22v-4a2 2 0 00-2-2h-2a2 2 0 00-2 2v4" /><path d="M2 7h20" /></svg>} />
        </div>
      )}
          ...
          <div className="bg-card border border-card-border rounded-lg px-6 py-4">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={revenueData}>
                ...
                <YAxis yAxisId="left" width={70} tick={{ fontSize: 11 }} stroke="#3B82F6" tickFormatter={(v) => `R$ ${v.toLocaleString("pt-BR", { notation: "compact" })}`} />
                <YAxis yAxisId="right" orientation="right" stroke="#10B981" width={40} tick={{ fontSize: 11 }} />
                <XAxis dataKey="name" stroke="#9CA3AF" />
                <Tooltip formatter={(value, name) => name === "revenue" ? [`R$ ${value.toLocaleString("pt-BR")}`, "Receita"] : [value, "Pedidos"]} />
